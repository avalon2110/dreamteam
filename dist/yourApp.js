(function() {
  ...
  var colors = d3.scale.pow()
    .exponent(3)
    .domain([2, 4, 6, 8, 10])
      .range(['white', 'red', 'maroon', 'fuchsia', 'purple']);
  ...
  .enter()
    ...
    $(document).ready(function() {
      var url = 'https://eonet.sci.gsfc.nasa.gov/api/v2.1/events?days=365?api_key=tSjNQ8SfpQxW6wT6IJCm1d1kyviuUFMNjdg1rwSe';
      var titleCords = [];
      var titlesArray = [];
      var events;
      ...
      $.ajax({
        ...
        success: function (jsonData) {
            ...
            for(event in events) {
              objTitle = jsonData.events[event].categories[0].title;

              if (titlesArray.indexOf(objTitle) === -1) {
                titlesArray.push(objTitle);
              }
            }

            for (var i = 0; i < titlesArray.length; i++) {
              var optionElement = '<option>' + titlesArray[i] + '</option>';
              $(optionElement).appendTo('#selectIssue');
            }
          },
      });

      $('select').on('change', function(){
        var optionIndex = titlesArray.indexOf($(this).val());
        titleCords = [];

        for (var i = 0; i < events.length; i++) {

          if (titlesArray[optionIndex] == events[i].categories[0].title){
            titleCords.push(events[i].geometries[0].coordinates);
          }

        }

        var ms = Date.now();
        var outTime = 1357011132440;

        titleCords = titleCords
          ...
          .map(el => {
            return {
              ...
              "lat" :el[0],
              "lng": el[1],
              "mag" :Math.floor(Math.random() * 10)
            }
          });
        console.log(titleCords);

          var data = glAr.reverse();
          console.log(123)
          ...
          d3.timer(function() {
            ...
            if (realDelta > 500) realDelta = 100;
